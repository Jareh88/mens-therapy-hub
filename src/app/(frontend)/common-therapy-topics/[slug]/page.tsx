...
export async function generateStaticParams() {
  ...
  const { docs: resources } = await payload.find({
    ...
    where: { type: { equals: "common-therapy-topics" } },
    pagination: false,
  });
...
export async function generateMetadata({
  ...
  const { docs: [resource] = [] } = await payload.find({
    ...
    where: {
      ...
      type: { equals: "common-therapy-topics" },
    },
    limit: 1,
  });
  ...
  const page = {
    meta: {
      title: resource?.meta?.title || "Getting Started | Men's Therapy Hub",
      description: resource?.meta?.description || "",
    },
    slug: ["/common-therapy-topics", `${resource.slug}`],
  };
